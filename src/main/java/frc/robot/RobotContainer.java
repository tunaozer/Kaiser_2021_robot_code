...
public class RobotContainer {
  ...
  private void configureButtonBindings() { //Button calls
    new JoystickButton(driver_Controller, XboxController.Button.kX.value).whenPressed(new ShootCommand(m_shooter));  //shooter button    //çalışmıyor düzeltilnesi lazım(efe'nin yorumu) 
    new JoystickButton(driver_Controller, XboxController.Button.kBumperLeft.value).whileHeld(new IndexCommand(m_indexer, m_shooter, m_intake));  //bu butona basılı tutunca intake ve shooterda yeni indexler yaratıyor

    new JoystickButton(driver_Controller, XboxController.Button.kA.value).whenPressed(new IntakeCommand(m_intake));//buna basıldığında  intake açılır/kapanır
