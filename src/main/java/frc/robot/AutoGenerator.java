...
import edu.wpi.first.wpilibj.trajectory.constraint.DifferentialDriveVoltageConstraint; //A class that enforces constraints on differential drive voltage expenditure based on the motor dynamics and the drive kinematics. Ensures that the acceleration of any wheel of the robot while following the trajectory is never higher than what can be achieved with the given maximum voltage.
...
public class AutoGenerator {
  ...
  private Command makeCenter3BallAuto() {
    ...
    new DifferentialDriveVoltageConstraint(
        new SimpleMotorFeedforward(DriveConstants.kS, //tatic gain
                                   DriveConstants.kV, //velocity
                                   DriveConstants.kA),
        Constants.DriveConstants.kDrive_Kinematics,
        10);

    TrajectoryConfig config = //Represents the configuration for generating a trajectory. This class stores the start velocity, end velocity, max velocity, max acceleration, custom constraints, and the reversed flag.
    new TrajectoryConfig(DriveConstants.kMax_Speed_Meters_Per_Second,
                         DriveConstants.kMax_Acceleration_Meters_Per_Second_Squared) //acceleration meter per seconds-squared
        .setKinematics(DriveConstants.kDrive_Kinematics) 
        .addConstraint(autoVoltageConstraint) //Adds a user-defined constraint to the trajectory.
        .setStartVelocity(0) //TrajectoryConfig.class'da bulunuyor
        .setEndVelocity(0)  //""""""
        .setReversed(false) //""""""Whether the trajectory should be reversed or not.
        ;
  ...
  private Command steal_trench() {
    ...
    TrajectoryConfig r_config = 
     new TrajectoryConfig(DriveConstants.kMax_Speed_Meters_Per_Second,
        ...
        //
        //Rotation2d==A rotation in a 2d coordinate frame represented a point on the unit circle (cosine and sine)
    ...
    , Collections.emptyList()
    , new Pose2d(-2.3, -4.59, new Rotation2d(-3.12)), config); //buraaya bi daha bak
   

    //alongwith==the commands to run in parallel
...
;
  ...
  private Command pathfromJSON() {
    ...
    try{
      Path trajectoryPath = Filesystem.getDeployDirectory().toPath().resolve(trajectoryJSON); //Class for interacting with the Filesystem, particularly, interacting with FRC-related paths on the system, such as the launch and deploy directories.
    ...
    m_drive.resetOdometry(0.76 , 0.76); //bir daha bak
