...
import edu.wpi.first.wpilibj.geometry.Pose2d; //Represents a 2d pose containing translational and rotational elements.
import edu.wpi.first.wpilibj.geometry.Rotation2d; //A rotation in a 2d coordinate frame represented a point on the unit circle (cosine and sine).
import edu.wpi.first.wpilibj.kinematics.DifferentialDriveOdometry; //Class for differential drive odometry. Odometry allows you to track the robot's position on the field over the course of a match using readings from 2 encoders and a gyroscope.Teams can use odometry during the autonomous period for complex tasks like path following. Furthermore, odometry can be used for latency compensation when using computer-vision systems.It is important that you reset your encoders to zero before using this class. Any subsequent pose resets also require the encoders to be reset to zero.
import edu.wpi.first.wpilibj.kinematics.DifferentialDriveWheelSpeeds; //Represents the wheel speeds for a differential drive drivetrain.
...
import edu.wpi.first.wpilibj.system.plant.DCMotor; //Holds the constants for a DC motor.
import edu.wpi.first.wpilibj.trajectory.Trajectory; //Represents a time-parameterized trajectory. The trajectory contains of various States that represent the pose, curvature, time elapsed, velocity, and acceleration at that point.
import edu.wpi.first.wpilibj.util.Units; //Utility class that converts between commonly used units in FRC.
